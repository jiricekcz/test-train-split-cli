...
class MemoryAssetManager(IAssetManager):
    ...
    def setAsset(self, assetIndex: int, asset: IAsset) -> None:
        if assetIndex < 0:
            raise AssetAppendException(asset, self, assetIndex, "Asset index must be valid index.")
        elif assetIndex > len(self.assets):
            raise AssetAppendException(asset, self, assetIndex, "Asset index out of range.")
        elif assetIndex == len(self.assets):
