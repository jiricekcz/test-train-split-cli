...
class RandomDistanceCalculator(IDistanceCalculator):
    def __init__(self):
        print(f"{Fore.YELLOW}WARINIG{Style.RESET_ALL}: A random distance calculator is being used! This should never happen unless you are testing the program!")
    def calculateDistances(self, distances: IDistanceMatrix, assets: IAssetManager, statusReport: Callable[[int, int, float], None] = ...) -> None:
